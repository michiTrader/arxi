...
func TestResolveUsesVerifiedSourceForReadersAndSeparationForWriters(t *testing.T) {
	...
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("resolved workspace requirements = %#v, want %#v: text-only work must stay filesystem-free, readers must resolve to the read-only profile so read-only-ness is carried by the workspace contract and not by the absence of a write grant, and writers need the strongest default separation", got, want)
	}
}
