...
//
// `arxi run start` refuses a non-simulated run today because there is no
	...
	if baseURL == "" {
		if !inTable {
			return Provider{}, fmt.Errorf("provider %q is not one of the providers "+
				...
				"    arxi provider add %s --base-url https://api.example.com/v1",
...
func validateName(name string) error {
	if strings.TrimSpace(name) == "" {
		return fmt.Errorf("a provider needs a name: it is what `arxi agent create " +
...
//
// The second check is the one that matters. `arxi provider add anthropic
...
func validateKeyEnv(name, env string) error {
	...
	if looksLikeASecret(env) {
		return fmt.Errorf("--api-key-env for provider %q looks like the key "+
			...
			"    export %s=<the key>\n"+
			"    arxi provider add %s --api-key-env %s",
