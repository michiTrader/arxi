...
func TestASelfReferentialRetireCannotFreezeARecord(t *testing.T) {
	...
	plain, err := Record{RecordID: "r1", Scope: sc, Kind: Approved, Sensitivity: Public, Body: "body A", Origin: "op"}.Seal()
	if err != nil {
		t.Fatal(err)
	}
	...
	written := writeRawVersion(t, s, Record{RecordID: "r1", Scope: sc, Kind: Approved, Sensitivity: Public,
...
func TestMutuallyRetiringRootsAreAVisibleForkNotAFrozenRecord(t *testing.T) {
	...
	sc := Scope{Principal: User, ID: "u1"}
	a, err := Record{RecordID: "r1", Scope: sc, Kind: Approved, Sensitivity: Public, Body: "body A", Origin: "op"}.Seal()
	if err != nil {
		t.Fatal(err)
	}
	b, err := Record{RecordID: "r1", Scope: sc, Kind: Approved, Sensitivity: Public, Body: "body B", Origin: "op"}.Seal()
	if err != nil {
		t.Fatal(err)
	}
	writeRawVersion(t, s, Record{RecordID: "r1", Scope: sc, Kind: Approved, Sensitivity: Public, Body: "body A",
		Origin: "op", Retires: []string{b.VersionID}})
	writeRawVersion(t, s, Record{RecordID: "r1", Scope: sc, Kind: Approved, Sensitivity: Public, Body: "body B",
