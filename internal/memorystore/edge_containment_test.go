...
func TestARetireCannotRemoveAnotherRecordsHead(t *testing.T) {
	...
	victim, err := s.Approve("victim", victimScope, Public, Operate, "important tenant memory", "op")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := s.Approve("attacker", attackerScope, Public, Operate, "attacker body", "op"); err != nil {
		t.Fatal(err)
	}
	// An imported/replicated/corrupt version of a different record names the
	// victim's head in Retires.
	writeRawVersion(t, s, Record{RecordID: "attacker", Scope: attackerScope, Kind: Approved, Sensitivity: Public, Purpose: Operate,
	...
	recs, _, err := s.Retrieve(Query{Purposes: []Purpose{Operate}, Scopes: []Scope{victimScope}})
	if err != nil {
		t.Fatal(err)
	}
	if len(recs) != 1 {
