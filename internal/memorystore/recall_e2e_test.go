...
func prepareFor(t *testing.T, store *memorystore.Store, runID string, scopes []memorystore.Scope) contextprep.Artifact {
	t.Helper()
	records, _, err := store.Retrieve(memorystore.Query{Scopes: scopes})
	if err != nil {
...
func TestMemoryWrittenInOneRunIsPresentedInTheNext(t *testing.T) {
	...
	if _, err := store.Approve("deploy-window", scopes[0], memorystore.Public, "deploys land on Tuesdays", "ana"); err != nil {
...
func TestRecalledMemoryArrivesOnTheMemoryChannel(t *testing.T) {
	...
	if _, err := store.Approve("deploy-window", scope, memorystore.Public, "deploys land on Tuesdays", "ana"); err != nil {
		t.Fatalf("approve: %v", err)
	}
...
func TestRecallIsScopedPerUserAcrossRuns(t *testing.T) {
	...
	if _, err := store.Approve("salary", ana, memorystore.Public, "ana negotiated a raise in March", "ana"); err != nil {
		t.Fatalf("approve: %v", err)
	}
...
func TestFrozenAndRetrievedMemoryCoexist(t *testing.T) {
	...
	records, _, err := store.Retrieve(memorystore.Query{Scopes: []memorystore.Scope{scope}})
	if err != nil {
