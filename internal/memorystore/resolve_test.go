...
func TestResolveHealsARootFork(t *testing.T) {
	...
	keep, err := s.Approve("r1", sc, "body A", "op")
	if err != nil {
		t.Fatal(err)
	}
	// A competing root arrives out of band, the case the write guard cannot stop.
	loser := writeRawVersion(t, s, Record{RecordID: "r1", Scope: sc, Kind: Approved, Body: "body B", Origin: "import"})
...
func TestResolveHealsASupersessionFork(t *testing.T) {
	...
	writeRawVersion(t, s, Record{RecordID: "r1", Supersedes: winner.Supersedes, Scope: sc,
		Kind: Approved, Body: "the branch that loses", Origin: "import"})
...
func TestResolveRefusesAVersionThatIsNotAHead(t *testing.T) {
	...
	sc := Scope{Principal: User, ID: "u1"}
	if _, err := s.Approve("r1", sc, "body A", "op"); err != nil {
		t.Fatal(err)
	}
	writeRawVersion(t, s, Record{RecordID: "r1", Scope: sc, Kind: Approved, Body: "body B", Origin: "import"})
...
func TestResolveRefusesAHealthyRecord(t *testing.T) {
	...
	sc := Scope{Principal: User, ID: "u1"}
	head, err := s.Approve("r1", sc, "body A", "op")
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveKeepingATombstoneLeavesRecordDeleted(t *testing.T) {
	...
	writeRawVersion(t, s, Record{RecordID: "r1", Scope: sc, Kind: Approved, Body: "resurrected body", Origin: "import"})
