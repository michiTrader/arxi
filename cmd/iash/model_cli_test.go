...
// with os.Exit.
//
// They share the harness in trigger_cli_test.go (TestMain, buildIash, iash,
...
func addProvider(t *testing.T, dir string, args ...string) result {
	t.Helper()
	r := iash(t, dir, append([]string{"provider", "add"}, args...)...)
	if r.code != 0 {
...
func TestAKeyPassedToTheFlagIsRefusedAndNothingIsWritten(t *testing.T) {
	...
	r := iash(t, dir, "provider", "add", "anthropic",
...
func TestEnablingAModelChangesWhatIsStored(t *testing.T) {
	...
	r := iash(t, dir, "model", "enable", "claude-opus-4-1")
	if r.code != 0 {
...
func TestDisablingTheModelEnabledAtRegistrationWorks(t *testing.T) {
	...
	if r := iash(t, dir, "model", "disable", "claude-sonnet-4-6"); r.code != 0 {
...
func TestEnablingTwiceSucceedsAndSaysItChangedNothing(t *testing.T) {
	...
	if r := iash(t, dir, "model", "enable", "claude-opus-4-1"); r.code != 0 {
...
func TestAnUnknownProviderWithoutAnEndpointIsRefusedAsMisuse(t *testing.T) {
	dir := workdir(t)
	r := iash(t, dir, "provider", "add", "together")
	if r.code != 2 {
...
`
	...
	q := iash(t, dir, "model", "enable", "local/claude-opus-4-1")
	if q.code != 0 {
...
func TestATypoInModelEnableSuggestsTheRealID(t *testing.T) {
	...
	r := iash(t, dir, "model", "enable", "claude-sonnet-4-5")
	if r.code != 1 {
...
func TestALiveRunIsNotLoggedAsASimulation(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			r := iash(t, dir, append(tc.args, "--dir", runDir)...)
			if r.code != 0 {
	...
	mux.HandleFunc("/v1/chat/completions", func(w http.ResponseWriter, r *http.Request) {
		if got := r.Header.Get("Authorization"); got != "" {
			...
			t.Errorf("a credential was sent to a loopback provider that has "+
				"none configured: %q\n"+
				"  consequence: a key crossing a link iash decided was safe "+
