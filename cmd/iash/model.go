...
// and they exist now for a concrete reason: `iash run start` refuses a real run
...
// goes stale — a flag added to the registry would be advertised by `iash
...
func cmdProvider(args []string) {
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "usage: iash provider add <name> "+
...
func cmdModel(args []string) {
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "usage: iash model list | enable <model> | disable <model>\n")
		os.Exit(2)
	}
	switch args[0] {
	case "list":
...
// cmdProviderAdd implements `iash provider add <name> [--base-url] [--api-key-env]`.
func cmdProviderAdd(args []string) {
	c := surface.Lookup("provider", "add")
	vals, err := parseInvocation(c, args)
	if err != nil {
		fmt.Fprintf(os.Stderr, "iash provider add: %v\n", err)
		os.Exit(2)
	}
	...
	if len(p.Models) == 0 {
		...
		fmt.Printf("  see what it offers: iash model list\n")
	}
}

// cmdModelList implements `iash model list`.
func cmdModelList(args []string) {
	c := surface.Lookup("model", "list")
	vals, err := parseInvocation(c, args)
	if err != nil {
		fmt.Fprintf(os.Stderr, "iash model list: %v\n", err)
		os.Exit(2)
	}
	...
	if len(rows) == 0 {
		...
		fmt.Printf("  register one: iash provider add anthropic --api-key-env "+
			"ANTHROPIC_API_KEY\n  known providers: %v\n", model.KnownNames())
		return
	}
...
func cmdModelEnable(args []string, on bool) {
	...
	vals, err := parseInvocation(c, args)
	if err != nil {
		fmt.Fprintf(os.Stderr, "iash model %s: %v\n", verb, err)
		os.Exit(2)
	}
	ref := vals["model"]
